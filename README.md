...
The login page is available at `http://localhost:8000/login` (default
credentials `admin`/`password`). After authenticating, the dashboard is
served at `http://localhost:8000/dashboard` and includes basic analytics
such as average latency and device status counts. An embedded form lets
operators register sandbox devices directly from the page, and a snippet
of JavaScript submits the data to the API and displays a confirmation
without reloading.
...
ports, warning lights, status and optional geolocation tags.

- `POST /sandbox/device` with JSON body
  `{ "name": "sim-router", "device_type": "router", "ports": 8, "latitude": -33.9, "longitude": 18.4 }`
...
- `POST /self-heal` to automatically recover devices in the sandbox.
  The routine applies QuantumShield risk scores before flipping a device
  online, ensuring remediation occurs only under acceptable thresholds.
...
The .NET SDK is also preloaded at FastAPI startup by invoking `dotnet --info`,
causing the service to fail fast if the SDK is unavailable.
...
For production deployments, the controller can target a Microsoft SQL Server instance. A sample schema and one-click setup are included:

```
make db-build
```

This target uses the connection string:

```
Server=SS-VEGA-DEV\SSVGASTD;Database=SS_NETWORK_MANAGER;User Id=ssqs_admin;Password=SSDv8P@QL2025;Trusted_Connection=True;Encrypt=True;
```

The script at `db/mssql_build.sql` now creates all tables used by the console—`devices`, `firmware`, `zero_touch`, `warnings`, and `audit_log`—providing a ready-made schema for integration tests.

The Python application will automatically construct a SQL Server connection string when the following environment variables are set:

```
MSSQL_SERVER=SS-VEGA-DEV\SSVGASTD
MSSQL_DB=SS_NETWORK_MANAGER
MSSQL_USER=ssqs_admin
MSSQL_PASSWORD=SSDv8P@QL2025
```

Alternatively, supply a custom SQLAlchemy `DATABASE_URL` (e.g., `mssql+pyodbc://...`). If no SQL Server details are provided, the application defaults to a local SQLite database for development and testing.
